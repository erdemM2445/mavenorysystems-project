...
async function boot(){
  ...
  $('#backToSignIn')?.addEventListener('click', () => { showConfirmPending(false); setAuthMode('signin'); });
...
async function submitAuth(){
  ...
  try{
    ...
    if(authMode === 'signin'){
      ...
      const siteUrl = (CFG.SITE_URL || window.location.origin).replace(/\/$/, '');
      result = await client.auth.signUp({
        email, password,
        options: {
          emailRedirectTo: siteUrl + '/app.html',
          data: { workspace_name: workspace, full_name: workspace }
        }
      });
      if(result.error) throw result.error;
      if(result.data?.session){
        state.session = result.data.session; state.user = result.data.user; await loadAll();
      } else {
        showConfirmPending(true, email);
      }
    }
...
function showConfirmPending(show, email){
  $('#authForm').classList.toggle('hidden', show);
  $('#confirmPending').classList.toggle('hidden', !show);
  if(email) $('#confirmEmail').textContent = email;
}
...
async function loadAll(){
  ...
  try{
    ...
    // FIX: sales query limited to last 90 days and max 500 rows to avoid memory/performance issues.
    const [profile, settings, products, sales, priceChanges] = await Promise.all([
      ...
      client.from('sales').select('*').gte('sale_date', daysAgoIso(90)).order('sale_date', { ascending: false }).limit(500),
...
// FIX: cancelled subscriptions keep full access until the billing period ends (plan stays 'full').
// Only 'expired' access_status revokes access (fired by Lemon Squeezy after the period ends).
function isFull(){
  const plan = state.profile?.plan;
  const status = state.profile?.access_status || 'active';
  if(plan !== 'full') return false;
  return ['active', 'past_due', 'cancelled'].includes(status);
}
...
function computations(){
  ...
  const rows = state.products.map(p => {
    ...
    // FIX: Decision priority reordered so a product with bad margins is never told "Order Now".
    // Priority (highest to lowest): Stop > Order Now > Raise Price > Test More > Keep
    let decision = 'Keep';
    ...
    // Stop overrides everything — a money-losing product should never be restocked
...
function render(){
  ...
  // FIX: Hide the destructive "Reset sample data" button for full paying users in production.
  // Show it only for free accounts so it functions as an onboarding helper.
  const seedBtn = $('#seedBtn');
  if(seedBtn) seedBtn.classList.toggle('hidden', isFull());

  // FIX: Hide the Upgrade button after a user has Full Planner access.
  // This prevents paid users from starting a duplicate checkout.
  const upgradeBtn = $('#upgradeBtn');
  if(upgradeBtn) upgradeBtn.classList.toggle('hidden', isFull());
...
// FIX: Show a notice when no products exist instead of rendering a broken empty select.
function renderSales(){
  if(!state.products.length){
    return `<div class="panel"><div class="empty">Add at least one product before logging sales. Go to the Products view to add your first SKU.</div></div>`;
  }
...
async function importCsv(){
  ...
  if(!rows.length) return toast('No valid rows found. Format: sku,sale_date,units_sold,revenue', 'error');
  ...
  if(error) return toast(error.message,'error');
  toast(`${mapped.length} sales rows imported.`, 'success'); $('#csvInput').value = ''; await loadAll();
}

async function seedSampleData(){
  if(!confirm('This will delete ALL your current products and sales, then load sample data.\n\nThis cannot be undone. Continue?')) return;
...
async function startCheckout(){
  ...
  if(isFull()) return toast('You already have Full Planner access.', 'success');
  return toast(BILLING_UNAVAILABLE_MESSAGE, 'info');
}
