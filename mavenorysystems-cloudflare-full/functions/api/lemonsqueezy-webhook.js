...
function statusToPlan(eventName, status) {
  ...
  const freeStatuses = new Set(["expired", "cancelled"]);
  if (eventName === "subscription_payment_success") return { plan: "full", access_status: "active", product_limit: null };
  if (activeStatuses.has(status)) return { plan: "full", access_status: status === "on_trial" ? "active" : status, product_limit: null };
  if (freeStatuses.has(status)) return { plan: "free", access_status: status, product_limit: 3 };
