...
export async function onRequestPost({ request, env }) {
  try {
    const billingUnavailableMessage = "Payments are temporarily unavailable while we complete final testing. For early access, please contact us at mavenorysystems@gmail.com.";

    // Payments are disabled by default for the current test phase.
    // To enable payments later, set BILLING_DISABLED=false in Cloudflare env and redeploy.
    if (env.BILLING_DISABLED !== "false") {
      return json({ error: billingUnavailableMessage }, 503);
    }
    ...
    if (missing.length) {
      return json({ error: `Missing environment variables: ${missing.join(", ")}` }, 500);
    }
    ...
    if (!user?.id || !user?.email) {
      return json({ error: "Sign in required before checkout." }, 401);
    }
      ...
      data: {
        ...
        relationships: {
          ...
          variant: {
            data: {
              type: "variants",
              id: String(env.LEMONSQUEEZY_VARIANT_ID)
            }
          }
        }
      }
    ...
    if (!response.ok) {
      return json({ error: "Lemon Squeezy checkout failed.", details: data }, response.status);
    }

    const url = data?.data?.attributes?.url;
    if (!url) {
      return json({ error: "Lemon Squeezy did not return a checkout URL.", details: data }, 502);
    }
